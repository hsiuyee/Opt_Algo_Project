...
def calculate_optimal_gap(fileName, file1, file2, output_file):
    ...
    # 只畫出前 90 epoch 的結果，每 3 個 epoch 繪製一次
    epochs = 90
    interval = 3
    x_ticks = np.arange(0, 31, 5)  # x 軸顯示 0, 5, ..., 30

    plt.figure(figsize=(10, 6))
    plt.plot(np.arange(0, 31), gap1[:epochs + 1:interval], label='SGD')
    plt.plot(np.arange(0, 31), gap2[:epochs + 1:interval], label='SRG')
